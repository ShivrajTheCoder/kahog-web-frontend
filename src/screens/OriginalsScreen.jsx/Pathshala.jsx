          ...
          {pathshalas.map((pathshala) => (
            ...
            >
              <img
                ...
                alt={pathshala.name}
                className="w-full h-48 object-cover" // Full width, fixed height, cropped if needed
              />
              <div className="p-4">
                <h2 className="text-xl font-bold">{pathshala.name}</h2>
                <p className="text-gray-700 mb-4">{pathshala.description}</p>
                <button
                  className="bg-green-500 text-white hover:bg-green-700 font-bold py-2 px-4 rounded focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 w-full" // Added w-full for full width
                  onClick={() => handleJoinPathshala(pathshala.id)}
                >
                  Join
                </button>
              </div>
